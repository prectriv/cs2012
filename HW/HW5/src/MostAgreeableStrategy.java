...
public class MostAgreeableStrategy implements I3VoteStrategy {
    ...
    /**
     * Calculates the winner based on which candidate has the most votes in any
     * category
     * 
     * @param votes a HashMap containing candidate names as keys and Votes objects
     *              for the value
     * @return an optional that contains a string if there is a valid winner, or
    ...
    public Optional<String> calculateWinner(HashMap<String, Votes> votes) {
        ...
        for (Map.Entry<String, Votes> i : votes.entrySet()) {
            ...
            int cv = Math.max(i.getValue().getFirstVotes(),
                    Math.max(i.getValue().getSecondVotes(), i.getValue().getThirdVotes()));
